...
static uint8_t mo_i_to_bcd(uint16_t n)
{
    if (n > 99)
...
short kbdmo_init() {
    ...
    a2560_irq_disable(INT_KBD_A2560K);
...
static unsigned char kbd_to_ansi(unsigned char modifiers, unsigned char c) {
    if ((c >= 0x80) && (c <= 0x98)) {
        ...
        if (modifiers & (KBD_MOD_SHIFT | KBD_MOD_CTRL | KBD_MOD_ALT | KBD_MOD_OS)) {
            ...
            code_bcd = mo_i_to_bcd(modifier_code);
